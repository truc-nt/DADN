    ...
    return (
        <TouchableOpacity
            className={`rounded-[20px] ${
                device.enabled ? 'bg-blue' : 'bg-white'
            } px-[4%] py-[4%] w-[45%] my-[3%]`}
            onPress={() =>
                /*navigation.navigate(
        ...
        >
            <MaterialCommunityIcons
                name={props.icon}
                size={30}
                color={`${device.enabled ? 'white' : 'black'}`}
            />
            <View className="mt-[20%]">
                <Text
                    style={{ fontFamily: 'LexendSemiBold' }}
                    className={`text-[17px] leading-[21px] ${
                        device.enabled ? 'text-white' : 'text-black'
                    }`}
                >
                    {props.name}
                </Text>
                ...
                >
                    {props.amount} thiết bị
                </Text>
            </View>
            <View className="flex-row justify-between mt-[6%]">
                <Text
                    style={{ fontFamily: 'LexendMedium' }}
                    className={`text-[17px] leading-[21px] self-center ${
                        device.enabled ? 'text-white' : 'text-grey'
                    }`}
                >
