...
exports.validateUserRegister = [
    check('username').trim().not()
        ...
        .isLength({min: 3, max: 10}).withMessage('Username must be within 3 to 10 characters!!!'),
    check('password').trim().not()
        .isEmpty().withMessage('Password is required!!!')
        .isLength({min: 7, max: 20}).withMessage('Password must be within 8 to 20 characters!!!'),
    check('repwd').trim().not().isEmpty().custom((repwd, {req}) => {
